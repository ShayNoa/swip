...
def copy_working_directory(working_dir: Path) -> None:
    ...
    """
    paths_to_copy = utils.get_rel_paths(working_dir, exclude_swip=True)
    for path in paths_to_copy:
        if path.is_file():
            utils.copy_file_to_staging(working_dir, path)
        if path.is_dir():
...
def handle_copying_to_staging(path: Path) -> None:
    ...
    if path.is_file():
        ...
        utils.copy_file_to_staging(working_dir, file_rel_path)
        return 
    copy_subdir_to_staging(working_dir, path)
