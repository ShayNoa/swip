...
def create_commit_metadata(commit_path: str, message: str, parents) -> None: 
    ...
    if not parents:
        parents = utils.get_head_commit_id() if path_to.references.exists() else None
...
def update_references_file(commit_id):
    ...
    be updated as well.
    """
    active = utils.get_active_branch()
    head = utils.get_head_commit_id()
...
def handle_references(commit_id: str) -> None:
    ...
    if not path_to.references.exists(): 
        utils.create_references_file(commit_id)
        return
