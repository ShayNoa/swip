...
def get_to_be_committed(last_commit_id: str) -> Set[Optional[Path]]:
    ...
    staging_paths = utils.get_rel_paths(path_to.staging_area, only_files=True) # no support for dirs currently!!
    diff = set()
...
def get_not_staged() -> Set[Optional[Path]]:
    ...
    """
    diff = set()
    staging_files = utils.get_rel_paths(path_to.staging_area, only_files=True)
    working_dir_files = utils.get_rel_paths(path_to.working_dir, only_files=True) # if dir is with a lottttt of files, get stuck here. 
    mutual_files = staging_files & working_dir_files
    diff = utils.get_diff_content_files(mutual_files, path_to.staging_area, path_to.working_dir)
    return diff
...
def get_untracked_files() -> Set[Optional[Path]]:
    ...
    """
    diff = set()
    working_dir_paths = utils.get_rel_paths(path_to.working_dir, exclude_swip=True)
    staging_paths = utils.get_rel_paths(path_to.staging_area)
    diff = working_dir_paths - staging_paths
    return diff


def get_status_info() -> NamedTuple: # A bug when trying to add hinting to the namedtuple
    ...
    """
    try:
        cur_commit = utils.get_head_commit_id()
    except FileNotFoundError:
        raise CommitRequiredError("Can't be executed before first commit")

    active_branch = utils.get_active_branch()
