...
from common.exceptions import CommitRequiredError, MergeError, NoBranchError
...
def validate_merge(branch_name):
    ...
    """
    if branch_name not in utils.get_all_branches_commits():
        raise NoBranchError('No such existing branch')

    status = get_status_info()
    if not utils.is_working_tree_clean(status):
        print_status(status)
...
def compare_dir_files(
    ...
    """
    branch_files = utils.get_rel_paths(branch_dir, only_files=True) 
    ancestor_files = utils.get_rel_paths(ancestor_dir, only_files=True) 
    added_files = branch_files - ancestor_files
    mutual_files = branch_files & ancestor_files
    changed_files = utils.get_diff_content_files(mutual_files, branch_dir, ancestor_dir) 
...
def update_staging_area(
    ...
    for file in files_to_copy:
        utils.copy_file_to_staging(branch_dir, file) 
...
def merge(branch_name: str) -> bool:
    ...
    head_commit = utils.get_head_commit_id() 
    branch_commit = utils.get_commit_by_branch(branch_name)
    ...
    utils.update_working_dir(path_to.staging_area) # what if there is untracked files?
    return True
